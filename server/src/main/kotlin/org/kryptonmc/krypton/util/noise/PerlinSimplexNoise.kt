...
class PerlinSimplexNoise private constructor(random: RandomSource, amplitudes: IntSortedSet) : SurfaceNoise {
    ...
    init {
        require(amplitudes.isNotEmpty()) { "No octaves provided! Octaves must be provided for the noise generator!" }
        val invFirst = -amplitudes.firstInt()
        val last = amplitudes.lastInt()
        ...
        if (last in 0..total && amplitudes.contains(0)) noiseLevels[last] = noiseLevel
        for (i in last + 1..total) if (i >= 0 && amplitudes.contains(total - i)) noiseLevels[i] = SimplexNoise(random) else random.skip(262)
        if (last > 0) {
            ...
            for (i in total - 1 downTo 0) if (i < total && amplitudes.contains(total - i)) noiseLevels[i] = SimplexNoise(genRandom) else genRandom.skip(262)
        }
