...
class Vec3dImpl(override val x: Double, override val y: Double, override val z: Double) : Vec3d {

    override fun add(x: Double, y: Double, z: Double): Vec3d {
        if (x == 0.0 && y == 0.0 && z == 0.0) return this
        return Vec3dImpl(this.x + x, this.y + y, this.z + z)
    }
    ...
    override fun multiply(factor: Double): Vec3d {
        ...
        return Vec3dImpl(x * factor, y * factor, z * factor)
    }
    ...
    override fun cross(x: Double, y: Double, z: Double): Vec3d = Vec3dImpl(this.y * z - this.z * y, this.z * x - this.x * z, this.x * y - this.y * x)

    override fun pow(power: Double): Vec3d = Vec3dImpl(x.pow(power), y.pow(power), z.pow(power))

    override fun abs(): Vec3d = Vec3dImpl(abs(x), abs(y), abs(z))

    override fun negate(): Vec3d = Vec3dImpl(-x, -y, -z)
    ...
    override fun normalize(): Vec3d {
        ...
        return Vec3dImpl(x / length, y / length, z / length)
    }
    ...
    companion object {
        ...
        @JvmField
        val ZERO: Vec3d = Vec3dImpl(0.0, 0.0, 0.0)
    }
}
