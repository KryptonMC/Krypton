...
class GossipUUIDFix(outputSchema: Schema, name: String) : NamedEntityFix(outputSchema, false, "Gossip for $name", References.ENTITY, name) {
    ...
    override fun fix(typed: Typed<*>): Typed<*> = typed.update(remainderFinder()) { entity ->
        entity.update("Gossips") { gossips ->
            val fixed = gossips.asStreamOpt().result().map { stream -> stream.map { it.replaceUUIDLeastMost("Target", "Target").orElse(it) } }
            DataFixUtils.orElse(fixed.map(gossips::createList), gossips)
        }
    }
}
