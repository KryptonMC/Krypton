...
import org.kryptonmc.krypton.util.converter.versions.V2514.replaceUUIDLeastMost

object V2516 {

    private const val VERSION = MCVersions.V20W12A + 1

    fun register() {
        val gossipUUIDConverter = object : StringDataConverter(VERSION) {
            override fun convert(data: MapType<String>, sourceVersion: Long, toVersion: Long): MapType<String>? {
                val gossips = data.getList("Gossips", ObjectType.MAP) ?: return null
                for (i in 0 until gossips.size()) {
                    gossips.getMap<String>(i).replaceUUIDLeastMost("Target", "Target")
                }
                return null
            }
        }
