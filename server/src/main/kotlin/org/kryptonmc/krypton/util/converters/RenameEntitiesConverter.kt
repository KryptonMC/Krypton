...
object RenameEntitiesConverter {
    ...
    fun register(version: Int, subVersion: Int, renamer: (String) -> String?) {
        MCTypeRegistry.ENTITY.addStructureConverter(version, subVersion) { data, _, _ ->
            val id = data.getString("id") ?: return@addStructureConverter null
            val converted = renamer(id)
            if (converted != null) data.setString("id", converted)
            null
        }
        RenameStringValueTypeHelper.register(version, subVersion, MCTypeRegistry.ENTITY_NAME, renamer)
    }
}
