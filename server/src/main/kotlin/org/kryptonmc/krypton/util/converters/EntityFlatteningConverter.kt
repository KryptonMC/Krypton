...
class EntityFlatteningConverter(private val paths: Array<out String>) : StringDataConverter(VERSION, 3) {
    ...
    override fun convert(data: MapType<String>, sourceVersion: Long, toVersion: Long): MapType<String>? {
        ...
        val idPath = paths[0]
        val dataPath = paths[1]
        val outputStatePath = paths[2]
        val blockId = if (data.hasKey(idPath, ObjectType.NUMBER)) data.getInt(idPath) else getBlockId(data.getString(idPath)!!)
        val blockData = data.getInt(dataPath) and 15
    ...
    companion object {
        ...
        private val BLOCK_NAME_TO_ID = mapOf(
            "minecraft:air" to 0,
        ...
        fun register() {
            MCTypeRegistry.ENTITY.addConverterForId("minecraft:falling_block", VERSION, 3) { data, _, _ ->
                val blockId = if (data.hasKey("Block")) {
                    data.getNumber("Block")?.toInt() ?: getBlockId(data.getString("Block")!!)
                } else {
                    data.getNumber("TileID")?.toInt() ?: (data.getByte("Tile").toInt() and 255)
                }
                val blockData = data.getInt("Data") and 15
        ...
        fun getBlockId(block: String) = BLOCK_NAME_TO_ID[block] ?: 0

        private fun register(id: String, vararg paths: String) {
            MCTypeRegistry.ENTITY.addConverterForId(id, EntityFlatteningConverter(paths))
        }
    }
}
