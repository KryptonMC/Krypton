...
object V2505 {
    ...
    private const val VERSION = MCVersions.V20W06A + 1

    fun register() {
        MCTypeRegistry.ENTITY.addConverterForId("minecraft:villager", VERSION) { data, _, _ ->
            val brain = data.getMap<String>("Brain") ?: return@addConverterForId null
            val memories = brain.getMap<String>("memories") ?: return@addConverterForId null
            memories.keys().forEach {
                val value = memories.getGeneric(it)!!
                val wrapped = NBTTypeUtil.createEmptyMap<String>()
                wrapped.setGeneric("value", value)
                memories.setMap(it, wrapped)
            }
            null
        }
