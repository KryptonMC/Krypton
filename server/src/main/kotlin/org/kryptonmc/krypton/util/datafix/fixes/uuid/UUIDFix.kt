...
sealed class UUIDFix(outputSchema: Schema, protected val typeReference: DSL.TypeReference) : DataFix(outputSchema, false) {
    ...
    companion object {
        ...
        fun Dynamic<*>.replaceUUIDString(old: String, new: String): Optional<Dynamic<out Any>> =
            createUUIDFromString(old).map { remove(old).set(new, it) }
        ...
        private fun Dynamic<*>.createUUIDTag(mostSignificant: Long, leastSignificant: Long) = Optional.of(
            createIntList(IntStream.of(
        ...
        private fun Dynamic<*>.createUUIDFromString(string: String): Optional<Dynamic<*>> = get(string).result().flatMap {
