...
class BlockEntityCustomNameFix(outputSchema: Schema, changesType: Boolean) : DataFix(outputSchema, changesType) {
    ...
    override fun makeRule(): TypeRewriteRule {
        ...
        return fixTypeEverywhereTyped("BlockEntityCustomNameToComponentFix", inputSchema.getType(References.BLOCK_ENTITY)) { typed ->
            typed.update(remainderFinder()) {
                val id = typed.getOptional(idFinder)
                if (id.isPresent && id.get() == "minecraft:command_block") it else it.fixCustomNameTag()
            }
        }
    }
}
