...
class AlignBitStorageFix(outputSchema: Schema) : DataFix(outputSchema, false) {
    ...
    override fun makeRule(): TypeRewriteRule {
        ...
        val sectionFinder = levelFinder.type().findField("Sections")
        val sectionsType = (sectionFinder as ListType<*>).element
        val sectionsFinder = typeFinder(sectionsType)
        ...
        return fixTypeEverywhereTyped("Align BitStorage fix", chunkType, outputSchema.getType(References.CHUNK)) { typed ->
            typed.updateTyped(levelFinder) { it.updateSections(sectionFinder, sectionsFinder, paletteFinder).updateHeightmaps() }
        }
    }
    ...
    private fun Dynamic<*>.updateBitStorage(current: Dynamic<*>, size: Int, bits: Int): Dynamic<*> {
        ...
        return createLongList(LongStream.of(*newArray))
    }
}
