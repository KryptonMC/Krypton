...
object RenameOptionsConverter {
    ...
    fun register(version: Int, subVersion: Int, renamer: (String) -> String?) {
        MCTypeRegistry.OPTIONS.addStructureConverter(version, subVersion) { data, _, _ ->
            data.keys().toList().forEach {
                val updated = renamer(it)
                if (updated != null) {
                    data.setGeneric(updated, data.getGeneric(it)!!)
                    data.remove(it)
                }
            }
            null
