...
object RenameAdvancementsConverter {
    ...
    fun register(version: Int, subVersion: Int, renamer: (String) -> String?) {
        MCTypeRegistry.ADVANCEMENTS.addStructureConverter(version, subVersion) { data, _, _ ->
            ArrayList(data.keys()).forEach {
                val updated = renamer(it) ?: return@forEach
                val value = data.getGeneric(it)!!
                data.remove(it)
