...
class GenerationRegion(
...
) : BlockAccessor by world {

    private val cache: MutableList<ChunkAccessor>
    val center: ChunkPosition
    private val size: Int
    private val seed = world.seed
    private val worldData = world.data
    private val random = world.random
    private val dimensionType = world.dimensionType
    ...
    fun hasChunk(x: Int, z: Int) = x >= firstPosition.x && x <= lastPosition.x && z >= firstPosition.z && z <= lastPosition.z

    fun getChunk(x: Int, z: Int) = getChunk(x, z, ChunkStatus.EMPTY, true)

    fun getChunk(x: Int, z: Int, status: ChunkStatus, warn: Boolean): ChunkAccessor? {
    ...
    private fun canWrite(x: Int, y: Int, z: Int): Boolean {
        val chunkX = x shr 4
        val chunkZ = z shr 4
