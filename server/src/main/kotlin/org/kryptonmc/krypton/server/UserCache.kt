...
class UserCache(private val path: Path) {
    ...
    fun load() {
        ...
        path.bufferedReader().use { reader ->
            gson.fromJson<JsonArray>(reader).forEach {
                val data = it.asJsonObject
                val entry = Entry.fromJson(data)
                if (entry == null) LOGGER.error("Failed to parse user cache entry $it!") else entries.add(entry)
            }
            entries.sortBy { entry -> entry.expiryDate }
        }
    }

    fun save() {
        val profiles = JsonArray()
        entries.take(1000).forEach {
            val data = JsonObject()
            it.writeToJson(data)
            profiles.add(data)
        }
        path.bufferedWriter().use { gson.toJson(profiles, it) }
    }
    ...
    data class Entry(val profile: GameProfile, val expiryDate: OffsetDateTime = OffsetDateTime.now()) {
        ...
        fun writeToJson(data: JsonObject) {
        ...
        companion object {
            ...
            fun fromJson(data: JsonObject): Entry? {
                ...
                val expiredDate = OffsetDateTime.from(if (expiredDateElement != null) DATE_FORMAT.parse(expiredDateElement) else OffsetDateTime.now().plus(1, ChronoUnit.MONTHS))
