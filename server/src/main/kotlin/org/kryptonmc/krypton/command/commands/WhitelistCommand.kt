...
object WhitelistCommand : InternalCommand {
            ...
            .then(literal<Sender>("list")
                .executes {
                    ...
                    if (whitelist.isEmpty()) {
                        ...
                        sender.sendMessage(translatable("commands.whitelist.list", whitelist.size.toString().toComponent(), whitelist.joinToString().toComponent()))
                    }
                    1
                })
                    ...
                    .executes {
                        ...
                        for (target in targets) {
                            ...
                            if (!whitelist.contains(target)) {
                                ...
                                sender.sendMessage(translatable("commands.whitelist.add.success", target.name.toComponent()))
                            } else {
                ...
                .then(argument<Sender, EntityQuery>("targets", GameProfileArgument())
                    .executes {
                        ...
                        for (target in targets) {
                            ...
                            if (whitelist.contains(target)) {
                                whitelist -= target
                                sender.sendMessage(translatable("commands.whitelist.remove.success", target.name.toComponent()))
                            } else {
                ...
                .then(argument<Sender, String>("ip", string())
                    ...
                    .executes {
                        ...
                        if (server.playerManager.whitlistedIps.contains(ip)) {
                            ...
                            sender.sendMessage(translatable("commands.whitelist.remove.success", ip.toComponent()))
                        } else {
                            sender.sendMessage(translatable("commands.whitelist.remove.failed"))
                        }
                        1
    ...
    private fun whitelistIp(server: KryptonServer, target: String, sender: Sender) {
        if (target.matches(BanIpCommand.PATTERN)) {
            ...
            val address = server.player(target)!!.address.stringify()
