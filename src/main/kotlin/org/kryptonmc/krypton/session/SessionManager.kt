...
class SessionManager(private val server: Server) {
    ...
    fun beginPlayState(session: Session) {
        ...
        session.player.location = spawnPosition.toLocation(world)
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            for (i in 0 until server.config.world.viewDistance.toArea()) {
                val chunkPosition = server.regionManager.chunkInSpiral(i, centerChunk.x, centerChunk.z)
                ...
                if (chunkRegion.x != regionX || chunkRegion.z != regionZ) {
                    chunkRegion = server.regionManager.loadRegionFromChunk(chunkPosition)
                }
